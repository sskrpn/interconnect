...
//class tcp_connection_master;
class tcp_server;

class task_manager : public std::enable_shared_from_this<task_manager> {
   private:
    TASK_STATE state = NOT_STARTED;
    std::unordered_map<uint64_t, std::weak_ptr<master_connection>> connections;
   ...
   public:
    ...
    void start();
    void add_connection(std::shared_ptr<master_connection> connection);
