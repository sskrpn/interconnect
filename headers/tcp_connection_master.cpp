...
void         master_connection::start()                       { do_read(); }
uint64_t     master_connection::get_threads()           const { return threads_; }
uint64_t     master_connection::get_id()                const { return id_; }
std::string& master_connection::get_message()                 { return message_;}
void         master_connection::set_message(std::string& msg) { message_ = msg; }
void         master_connection::set_id(uint64_t& id)          { id_ = id; }
tcp::socket& master_connection::socket()                      { return socket_; }

master_connection::master_connection(tcp::socket&& socket,
                                     std::shared_ptr<task_manager> task_manager) 
                                     : tcp_connection(std::move(socket)), manager_(task_manager) {}

void master_connection::do_read() {
    if (first_connect) {
        first_connect = false;
...
void master_connection::respond() {
...
void master_connection::read_threads() {
...
void master_connection::read_results() {
    auto manager_ptr = manager_.lock();
    if (!manager_ptr) {
        std::cerr << "[tcp_connection] ";
